...
static inline void atomic_compiler_barrier(memory_order order)
{
...
#define atomic_store32(DST, SRC, ORDER)                                        \
    if (ORDER == memory_order_seq_cst)                                         \
    {                                                                          \
        InterlockedExchange((long volatile *)(DST), (long)(SRC));              \
    }                                                                          \
    else                                                                       \
    {                                                                          \
        *(DST) = (SRC);                                                        \
...
#define atomic_readX(SRC, DST, ORDER) *(DST) = *(SRC);
...
#define atomic_read64(SRC, DST, ORDER)                                         \
    __pragma(warning(push)) __pragma(warning(disable : 4047)) * (DST) =        \
        InterlockedOr64((int64_t volatile *)(SRC), 0);                         \
...
#define atomic_add8(RMW, ARG, ORIG, ORDER)                                     \
    *(ORIG) = _InterlockedExchangeAdd8((char volatile *)(RMW), (char)(ARG));

#define atomic_add16(RMW, ARG, ORIG, ORDER)                                    \
    *(ORIG) = _InterlockedExchangeAdd16((short volatile *)(RMW), (short)(ARG));

#define atomic_add32(RMW, ARG, ORIG, ORDER)                                    \
    *(ORIG) = InterlockedExchangeAdd((long volatile *)(RMW), (long)(ARG));
 ...
 * Do the same here. */
static inline bool atomic_compare_exchange8(int8_t volatile *dst,
                                            int8_t *expected, int8_t src)
{
    int8_t previous =
        _InterlockedCompareExchange8((char volatile *)dst, src, *expected);
...
static inline bool atomic_compare_exchange16(int16_t volatile *dst,
                                             int16_t *expected, int16_t src)
{
...
static inline bool atomic_compare_exchange32(int32_t volatile *dst,
                                             int32_t *expected, int32_t src)
{
    int32_t previous =
        InterlockedCompareExchange((long volatile *)dst, src, *expected);
    if (previous == *expected)
    {
        return true;
    }
...
static inline bool atomic_compare_exchange64(int64_t volatile *dst,
                                             int64_t *expected, int64_t src)
{
...
static inline bool atomic_compare_unreachable() { return true; }
         ...
         : (sizeof *(DST) == 2                                                 \
                ...
                : (sizeof *(DST) == 4                                          \
                       ...
                       : (sizeof *(DST) == 8                                   \
                              ...
                              : ovs_fatal(0, "atomic operation with size "     \
...
#define atomic_flag_test_and_set(FLAG) (bool)InterlockedBitTestAndSet(FLAG, 0)

#define atomic_flag_test_and_set_explicit(FLAG, ORDER)                         \
    atomic_flag_test_and_set(FLAG)

#define atomic_flag_clear_explicit(FLAG, ORDER) atomic_flag_clear()
#define atomic_flag_clear(FLAG) InterlockedBitTestAndReset(FLAG, 0)
